    ...
    async (loggedUserId: number) => {
        ...
        const response = await axios.get(
            `${import.meta.env.VITE_REACT_APP_API_URL}${url}`,
                config);
        return response.data.map(message => ({...message, date: new Date(message?.date)}));
    }
)

export const getConversationMessages = createAsyncThunk(
    "conversation/",
    async ({receiverId, senderId}: {receiverId: number, senderId: number}) => {
...
export const sendMessage = createAsyncThunk(
    ...
    async (payload: MessagePayload) => {
        ...
        const response = await axios.post(`${import.meta.env.VITE_REACT_APP_API_URL}${url}`, payload, config);
...
export const searchUser = createAsyncThunk(
    ...
    async (username: string) => {
        ...
        const response = await axios.get(`${import.meta.env.VITE_REACT_APP_API_URL}${url}`,
            config);
        return response.data;
    }
)
